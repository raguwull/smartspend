...
@Component({
  selector: 'app-comparison-trend-graph',
  standalone: true,
  imports: [],
  templateUrl: './comparison-trend-graph.component.html',
  styleUrl: './comparison-trend-graph.component.css',
})
export class ComparisonTrendGraphComponent implements OnInit {
  ...
  loadchartdata() {
    ...
    this.spendingService.getWeeklySpending().subscribe((data) => {
      ...
      if (this.linechartdata) {
        ...
        this.cur_month_amountdata = this.linechartdata.map(
          (o: any) => o.current_month_amount
        );
        this.prev_month_amountdata = this.linechartdata.map(
          (o: any) => o.previous_month_amount
        );

        this.Renderlinechart(
          this.labeldata,
          this.cur_month_amountdata,
          this.prev_month_amountdata,
          'linechart',
          'line'
        );
      }
  ...
  Renderlinechart(
    labeldata: any,
    cur_month_amountdata: any,
    prev_month_amountdata: any,
    chartid: string,
    charttype: any
  ) {
    try {
